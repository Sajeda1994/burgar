<div class="modal fade" id="editappet-{{$drink->id}}" tabindex="-1" role="dialog" aria-labelledby="verticalModalTitle"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="verticalModalTitle"> {{ __('trans.Edit Drinks') }}</h5>
            ...
            <form action="{{ route('drink.update',$drink->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <input type="hidden" name="editdrink" value="{{$drink->id}}">

                    <div class="row my-3">
                        <div class="col-6">
                            <label for="" class="label-form">{{ __('trans.Name In Arabic') }}</label>
                            <input type="text" name="namear" value="{{$drink->getTranslation('name','ar')}}" class="form-control">
                        </div>
                        <div class="col-6">
                            <label for="" class="label-form">{{ __('trans.Name In English') }}</label>
                            <input type="text" name="nameen" value="{{$drink->getTranslation('name','en')}}" class="form-control">
                        </div>
                    </div>
                    <div class="row my-3">
                        <div class="col-6">
                            <label for="" class="label-form">{{ __('trans.Description In Arabic') }}</label>
                            <textarea type="text" name="descar" class="form-control">{{$drink->getTranslation('description','ar')}}</textarea>
                        </div>
                        <div class="col-6">
                            <label for="" class="label-form">{{ __('trans.Description In English') }}</label>
                            <textarea type="text" name="descen" class="form-control">{{$drink->getTranslation('description','en')}}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="row my-3">
                        <div class="col-6">
                            ...
                            <input type="text" name="price" value="{{$drink->price}}" class="form-control">
                        </div>
                        <div class="col-6">
